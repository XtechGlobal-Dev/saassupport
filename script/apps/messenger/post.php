...
<<<<<<< HEAD
            if (!$conversation_id) {
                ...
                $message_previous = sb_db_get('SELECT message, attachments, creation_time FROM sb_messages WHERE conversation_id = ' . $conversation_id . ' ORDER BY id DESC LIMIT 1');
                $message_previous_text = trim(sb_isset($message_previous, 'message'));
