 ...
 * ==========================================================
 *
 * Slack app. © 2017-2025 board.support. All rights reserved.
 *
 */
...
function sb_send_slack_message($user_id, $full_name, $profile_image = SB_URL . '/media/user.png', $message = '', $attachments = [], $conversation_id = false, $channel = false) {
    ...
    if (!$channel) {
        $channel = $user_id == -1 || !$user_id ? [] : sb_get_slack_channel($user_id, $token, 0, false, $department_id);
        if (!$channel[0]) {
    ...
    if ($message[1])
        $data['blocks'] = json_encode($message[1], JSON_INVALID_UTF8_IGNORE | JSON_UNESCAPED_UNICODE);
...
function sb_slack_can_send($conversation_id) {
    ...
    return sb_get_setting('slack-active') && (!defined('SB_DIALOGFLOW') || (!sb_get_setting('dialogflow-active') && !sb_get_multi_setting('google', 'dialogflow-active')) || !sb_dialogflow_get_human_takeover_settings()['active'] || sb_dialogflow_is_human_takeover($conversation_id));
}
...
function sb_slack_get_users($token = false) {
    ...
    $members = sb_isset($response, 'members');
    $users = [];
    if ($members) {
        ...
        sb_error('no-slack-members', 'sb_slack_get_users', 'Slack members not found: ' . json_encode($response));
    }
    return $users;
}
