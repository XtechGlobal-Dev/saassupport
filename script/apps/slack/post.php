    ...
    if (isset($response['event'])) {
        ...
        if (isset($response['type']) && $response['type'] == 'message' && $subtype != 'channel_join' && (!$subtype || $subtype == 'file_share') && ($response['text'] != '' || (is_array($response['files']) && count($response['files']) > 0))) {
            ...
            if ($user_id) {
                ...
                // Messaging apps
                $conversation_details = defined('SB_MESSENGER') || defined('SB_WHATSAPP') || defined('SB_TWITTER') || defined('SB_VIBER')|| defined('SB_ZALO') || defined('SB_LINE') || defined('SB_TELEGRAM') || defined('SB_WECHAT') ? sb_db_get('SELECT source, extra FROM sb_conversations WHERE id = ' . $conversation_id) : false;
        ...
        switch ($subtype) {
            case 'message_deleted':
                ...
                sb_db_query(($online ? 'UPDATE sb_messages SET message = "", attachments = "", payload = "{ \"event\": \"delete-message\" }", creation_time = "' . sb_gmt_now() . '"' : 'DELETE FROM sb_messages') . ' WHERE (user_id = ' . $agent_id . ' OR user_id = ' . $user_id . ') AND conversation_id = "' . $last_message['conversation_id'] . '" AND ' . ($previous_message ?  'message = "' . $previous_message . '"' : 'attachments LIKE "%' . sb_db_escape($response['previous_message']['attachments'][0]['title']) . '%"') .' LIMIT 1');
                break;
            case 'message_changed':
                $agent_id = sb_db_escape(sb_slack_api_agent_id($response['previous_message']['user']));
                sb_db_query('UPDATE sb_messages SET message = "' . sb_db_escape(sb_slack_response_message_text($response['message']['text'])) . '", creation_time = "' . sb_gmt_now() . '" WHERE user_id = ' . $agent_id . ' AND payload LIKE "%' .  sb_db_escape($response['message']['ts']) . '%" LIMIT 1');
                break;
