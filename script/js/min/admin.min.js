    ...
    */
    ...
    var SBDashboard = {
        ...
        get: function (onSuccess, ticket_id = false, categories = false, full = true, language = false) {
            ...
            SBF.ajax({
                ...
                const months = Object.keys(response[1].resolved_ticket_data);
                // Get values array
                const values = Object.values(response[1].resolved_ticket_data);
                $('.ticket-resolved').html(response[1].resolved_tickets_count);

                const lastWeekResolvedSum = values.reduce((acc, curr) => acc + curr, 0);
                ...
                console.log('tt',values,response[1].resolved_tickets_count,resolvedBeforeLasteWeek,resolvedTicketspercentageIncrease);

                $('.total-resolved-tickets-increase').html(resolvedTicketspercentageIncrease);
                ...
                $('.total-tickets-increase').html(percentageIncrease);
            ...
            SBF.ajax({
                ...
                console.log('uu',values2,months2,);

                $('.total-conversations-increase').html(conversationsTicketspercentageIncrease);
    ...
    0
            ...
            SBF.ajax({
                ...
                const months = Object.keys(response[1].data);
                // Get values array
                const values = Object.values(response[1].data);
            ...
            SBF.ajax({
                ...
                $('.total-users-increase').html(percentageIncrease);

                console.log(response[1].total_users_count,lastWeekUsersSum,percentageIncrease,userValues);
                
                const ticket_resolvedCtx = document.getElementById('active_users_chart').getContext('2d');
                const gradient4 = ticket_resolvedCtx.createLinearGradient(0, 0, 0, 200);
                gradient4.addColorStop(0, 'rgba(231, 110, 241, 0.2)');
                gradient4.addColorStop(1, 'rgba(231, 110, 241, 0)');
                new Chart(ticket_resolvedCtx, {
            ...
            SBF.ajax({
                ...
                $(response[1].data).each(function()
                {
                    html += `<li class="msg d-flex g-3 mb-6">
                        ...
                        <div class="col-9">
                            ...
                            <div class="sub_head2 mb-2">${this.message}</div>
                            <small class="text-muted">${SBF.beautifyTime(this.creation_time, true)}</small>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <button class="reply-btn"><a href="?conversation=${this.conversation_id}" target="_blank">Reply</a></button>
                        </div>
                    </li>`;
                });
                ...
                setTimeout(function() {
                     $('.sb-area-dashboard .recent-messages').html(html !== '' ? html : '<span class="d-block" style="text-align: center;height: auto;">No message found</span>');
        ...
        fetchCustomerOverview: function()
        {
            ...
            SBF.ajax({
                ...
                if(response[1][0] = 'success')
                {
                    ...
                    {
                        $("#chart-container").html('No data available to graph.');
                    }
                }
        ...
        renderNewUsersChart: function (from, to) {
            const ctx = document.getElementById('new_users_chart');
            if (!ctx) {
            ...
            SBReports.getData("users", from, to, (response) => {
                ...
                const new_usersCtx = ctx.getContext('2d');
    ...
    var SBTicket = {
        ...
        show: async function (ticket_id) {
            ...
            SBF.ajax({
                ...
                //                                         <img src="${value.file_path}" class="img-thumbnail" style="max-height: 100px;" alt="${value.original_filename}">
                //                                     </div>
        ...
        getRow: function (ticket) {
            if (ticket) {
                ...
                ticket.ticket_tags.forEach(tag => {
                    const property = tag.color ? `background-color:${tag.color}` : `border: 1px solid #000;color:#000`;
                    rowHtml += `<span class="badge" style="${property}; margin-right: 5px;">${tag.name}</span>`;
                });
                rowHtml += `</td>`;
                  
                if (tickets_table.find('th[data-field="department"]').length) {
                    rowHtml += `<td class="sb-td-department">${ticket.department}</td>`;
                }
                rowHtml += `<!--td class="sb-td-service">${ticket.service_name}</td-->
                <td class="sb-td-contact">${ticket.contact_name ?? 'Guest'}</td>
                <td class="sb-td-status1 no-click status-dropdown">
                    <button class="status-btn currentStatusBtn" data-id="${ticket.id}" style="background-color: ${SBDashboard.lightenColor(ticket.status_color, 40)};color: ${ticket.status_color}">
                        <span class="status-dot" style="background-color: ${ticket.status_color};"></span> ${ticket.status_name}
                        <span class="arrow">▼</span>
                    </button>
                    ${this.getStatusesList()}
                </td>
                <td class="sb-td-priority no-click status-dropdown">
                    <button class="status-btn currentPriorityBtn" data-id="${ticket.id}" style="background-color: ${SBDashboard.lightenColor(ticket.priority_color, 40)};color: ${ticket.priority_name}">
                        <span class="status-dot" style="background-color: ${ticket.priority_color};"></span> ${ticket.priority_name}
                        <span class="arrow">▼</span>
                    </button>
                    ${this.getPrioritiesList()}
                </td>
                <td class="sb-td-last_reply">${ticket.last_reply ?? ''}</td>
                <!--td class="sb-td-conversation">#${ticket.conversation_id}</td-->
                <td>${SBF.beautifyTime(ticket.creation_time, true)}</td>
                </tr>`;
        ...
        updateRow: function (ticket) {
            ...
            if (row.length) {
                /*let menu_active = tickets_table_menu.find('.sb-active').data('type');
        ...
        updateMenu: function (action = 'all', type = false) {
            let ticket_status = ['all', 'open', 'in-progress', 'hold', 'waiting', 'answered', 'closed'];
            if (action == 'all') {
                SBF.ajax({
        ...
        updateMenuItem: function (action = 'set', type = false, count = 1) {
            ...
            //tickets_table_menu.find(`[data-type="all"] span`).html(`(${count})`).attr('data-count', count);
        },
        ...
        populate: function (response) {
            ...
            //tickets_area.find(is_category ? '.ul-categories' : '.ul-articles').html(code);
            // tickets_area.find(is_category ? '.ul-categories > li' : '.ul-articles > li').eq(0).click();
            // if (!items.length) {
            //      $(is_category ? articles_content_categories : articles_content).sbLoading(false).addClass('sb-hide');
            // }
        },
        ...
        loadComments : function (ticket_id) {
            SBF.ajax({
                ...
                // attachEditListeners();
                    ...
                    //         const commentDate = comment.created_at.split(' ')[0];
                    //         if (commentDate !== lastDate) {
                    //             html += `<div class='text-center my-2'><span class='badge bg-secondary' style='font-size:13px;'>${formatDateLabel(comment.created_at)}</span></div>`;
                    //             lastDate = commentDate;
                    //         }
        ...
        openTicket: async function (ticket_id) {
            console.log("openTicket called with ticket_id:", ticket_id);
           ...
           // loadingGlobal();
            ...
            await SBF.ajax({
                ...
                document.querySelector('.sb-area-ticket-detail .tc_back #ticket-subject').value  = response.subject;
                ...
                if (attachmentsHtml != '') {
                    console.log('imagesssss:',attachmentsHtml);
                    $('#existing-file-preview-container').removeClass('d-none');
                    $('#existing-file-preview-container #current-attachments').html(attachmentsHtml);
        ...
        openTicketArea: function (ticket_id) {
            ...
            $('.sb-area-ticket-detail .header-left .title').html('Ticket Detail');
    ...
    */
    ...
    var SBReports = {
        ...
        initChart: function (data, type = 'line', label_type = 1) {
            ...
            // console.log("ctx type- ", type, labels, values, "label_type>",label_type);
    ...
    var SBUsers = {
        ...
        getRow: function (user) {
            if (user instanceof SBUser) {
                ...
                return `<tr data-user-id="${user.id}" data-user-type="${user.type}"><td><input type="checkbox" /></td><td class="sb-td-profile"><a class="sb-profile"><img loading="lazy" src="${user.image}" data-name="${user.name}"/><div class="user-initials" data-value="edit-profile" style="display: none;"><span class="initials"></span></div><span>${user.name}</span></a></td>${code}<td class="sb-td-email">${user.get('email')}</td><td class="sb-td-ut">${sb_(user.type)}</td><td>${SBF.beautifyTime(user.get('last_activity'), true)}</td><td>${SBF.beautifyTime(user.get('creation_time'))}</td></tr>`;
            } else {
                ...
                if (conversation_id != -1) {
                    ...
                    activeUser().getFullConversation(conversation_id, (response) => {
                        ...
                        if (converted == 1) {
                            ...
                            $('#convert-to-ticket span').html('Converted to ticket');
                            document.querySelector('#convert-to-ticket').setAttribute('data-sb-tooltip', 'Converted to ticket');
                            ...
                            $('#convert-to-ticket span').html('Convert to a ticket');
                            document.querySelector('#convert-to-ticket').setAttribute('data-sb-tooltip', 'Convert to a ticket');
    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(header).on('click', ' .sb-admin-nav a', function () {
            ...
            //// change header title dynamically
            $('.sb-area-' + active_admin_area + ' .header-left .title').html(capitalizeFirstLetter(active_admin_area));
        ...
        $(conversations_filters).on('click', 'li', function (e) {
            ...
            setTimeout(() => {
                ...
                SBF.ajax({
                    ...
                    showInitials('sb-scroll-area', 'sb-profile');
                    console.log('Conversations filtered: ' + response.length);
                });
            }, 100);
        });
        ...
        */
        ...
        $(document).on('click','.currentStatusBtn,.currentPriorityBtn', function (e) {
            e.stopPropagation();
            $(this).next().toggle(); // Show/hide dropdown
        });
        ...
        $(settings_area).on('click', '.sb-new-ticket-custom-field', function () {
            ...
            ticket_custom_fields_edit_box.find('#optionsContainer').html('');
        });
        ...
        $(settings_area).on('click', '.edit-ticket-status', function () {
            ...
            SBF.ajax({
                ...
                document.querySelector('.sb-ticket-statuses-edit-box #status_color input').value = response.color;
        ...
        $("#save-ticket-attachments").on("click", async function () {
            try {
                ...
                console.log('files_count',attachmentsCount);
        ...
        $('.sb-save-ticket').on('click',function()
        {
            ...
            let ticketData = {assignee,department,tags,priority, description, customField };
        ...
        $(ticket_edit_box).on('click', '.sb-save', function () {
            ...
            let ticketData = { ...ticketDataPart1, ...ticketDataPart2, description, conversation_id, attachments, withoutContact, customField };
