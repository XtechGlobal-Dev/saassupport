        ...
        loadCustomFields: async function () {
            try {
                ...
                fields.forEach(field => {
                    const fieldHtml = this.getFieldHtml(field);
...
// Update form fields
                ...
                const select = document.querySelector('#contact_id select');
                console.log(select); // Should log the <select> element
                console.log(response.contact_id); // Should log a value like "2"
                select.value = response.contact_id;
                console.log(select.value); // Should reflect the assigned value

                document.querySelector('#assigned_to select').value = response.assigned_to;
                ...
                document.querySelector('#description textarea').value = response.description;
                document.querySelector('#ticket_id').value = response.id;

                /*document.getElementById('subject').find('input').value = data.id;
                document.getElementById('editSubject').value = data.subject;
                document.getElementById('editContactId').value = data.contact_id;*/
        ...
        getRow: function (ticket) {
            if (ticket) {
                ...
                <td><input type="checkbox" /> ${ticket.id}</td>
                <td class="sb-td-subject">${ticket.subject}</td>
                <td class="sb-td-tags">${ticket.tags ?? ''}</td>
                <td class="sb-td-department">${ticket.department_name }</td>
                <td class="sb-td-service">${ticket.service_name}</td>
                <td class="sb-td-contact">${ticket.contact_name ?? 'Guest'}</td>
                <td class="sb-td-status"><span class="span-border" style="color:${ticket.status_color};border:1px solid ${ticket.status_color};">${ticket.status_name}</span></td>
                <td class="sb-td-priority"><span class="span-border" style="color:${ticket.priority_color};border:1px solid ${ticket.priority_color};">${ticket.priority_name}</span></td>
            ...
            if (action == 'all') {
                SBF.ajax({
                    ...
                    for (var i = 0; i < ticket_status.length; i++) {
                        this.updateMenuItem('set', ticket_status[i], response[ticket_status[i]]);
                    }
                });
            } else {
    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(conversations_area).on('click', '.sb-top ul a', function () {
            ...
            if (status_code == 6) {
                //alert('do function here'+selected_conversations[0].id);
        ...
        */
        ...
        // Ticket add Custom fields

        // $(admin).on('click', '#save-custom-fields', function () {
        //      let ticketCustomFields = SBTicket.getAll($('#customFieldForm .modal-body'));
        //      alert(ticketCustomFields);
        //     SBF.ajax({
        //         function: 'update-conversation-extra',
        //         conversation_id: SBChat.conversation.id,
        //         extra: cc ? cc : 'NULL'
        ...
        $(settings_area).on('click', '.sb-new-ticket-custom-field', function () {
            console.log('add custom field popup');
            ticket_custom_fields_edit_box.addClass('sb-ticket-new');
            ticket_custom_fields_edit_box.find('.sb-top-bar .sb-ticket span').html(sb_('Add new ticket'));
            ticket_custom_fields_edit_box.find('.sb-top-bar .sb-save').html(`<i class="sb-icon-check"></i>${sb_('Add Ticket')}`);
        ...
        /*$("#save-custom-fields").on("click", function () {
            ...
            // let ticketCustomFields = SBTicket.getAll($("#customFieldForm"));
        ...
        $("#save-custom-fields").on("click", async function () {
            try {
                ...
                inputs.forEach(el => {
                    ...
                    //const fieldType = el.name == 'type' && (el.value == 'select' || el.value == 'checkbox') ? 'required' : '';
                ...
                const response = await new Promise((resolve, reject) => {
                    SBF.ajax({
                        ...
                        if (res && res.error) {
                            ...
                            resolve(res);
                            admin.sbHideLightbox();
                            infoBottom('Custom field deleted');
                        }
                    });
                });
        ...
        $(ticket_edit_box).on('click', '.sb-save', function () {
            ...
            const description =  [];
            description.push(ticketDataPart3);
            ...
            fields.forEach(field => {
            const name = field.name || field.id; // fallback if name missing
            ...
            let ticketData = {...ticketDataPart1, ...ticketDataPart2, description, conversation_id, customField};
            ...
            SBF.ajax({
                ...
                /*$(ticket_edit_box).find('.sb-info').html(sb_(response.msg)).sbActive(true);
                clearTimeout(timeout);
                timeout = setTimeout(function () {
                    //$(area).find('.sb-info').sbActive(false);
                    $(ticket_edit_box).find('.sb-info').style.background = 'Green';
                }, 7000);*/
                ...
                activeUser().update(() => {
                    ...
                    if (new_user) {
                        ticket_edit_box.find('.sb-profile').setProfile(sb_('Add new ticket'));
                    }
                    $(this).sbLoading(false);
                    if (!new_user) {
                        admin.sbHideLightbox();
                    }
                    infoBottom(new_user ? 'New Ticket Created' : 'Ticket updated');
                });*/
