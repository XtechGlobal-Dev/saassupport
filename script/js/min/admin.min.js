...
var SBTicket = {
        ...
        show: function (ticket_id) {
            ...
            this.get((response) => {
                ...
                response = response[0];
                articles_content.sbLanguageSwitcher(this.translations.get(response.parent_id ? response.parent_id : article_id), 'articles', response.language);
                articles_content.attr('data-id', ticket_id);
                tickets_area.find('.sb-article-title input').val(response.title);
                tickets_area.find('.sb-article-link input').val(response.link);
                tickets_area.find('#sb-article-id').html(`ID <span>${ticket_id}</span>`);
                tickets_area.find('.sb-article-categories').setClass('sb-hide', response.language);
                if (editor_js || articles_area.find('#editorjs').length) {
                    editorJSLoad(response.editor_js ? (isString(response.editor_js) ? JSON.parse(response.editor_js) : response.editor_js) : response.content);
                } else {
                    articles_area.find('.sb-article-content textarea').val(response.content);
    ...
    $(document).ready(function () {
        ...
        */
         ...
         $(tickets_table).on('click', 'td:not(:first-child)', function () {
            //SBProfile.show($(this).parent().attr('data-ticket-id'));
        });
        ...
        $(tickets_area).on('click', '.sb-new-ticket', function () {
            ...
            SBProfile.boxClasses(ticket_edit_box);
        ...
        $(ticket_edit_box).on('click', '.sb-save', function () {
            ...
            let ticket_id = ticket_edit_box.attr('data-ticket-id');

            // Get settings
            let settings = SBTicket.getAll(ticket_edit_box.find('.sb-details'));
            let settings_extra = SBTicket.getAll(ticket_edit_box.find('.sb-additional-details'));
            let output = {};
            ...
            //const errorCount = ticket_edit_box.find('#subject input').val().length < 1 || ticket_edit_box.find('#priority_id select').val().length < 1 || ticket_edit_box.find('#status_id   select').val().length < 1  ? 1 : 0;
            // Errors check
            //if (SBTicket.errors(ticket_edit_box)) {
            if(!isFormValid) {
            ...
            console.log(settings, settings_extra);
            // Save the settings
            SBF.ajax({
