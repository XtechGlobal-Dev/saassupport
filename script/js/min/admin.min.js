    ...
    var SBTicket = {
        ...
        getRow: function (ticket) {
            if (ticket) {
                ...
                <div class="dropdown">
                    <button class="btn btn-sm dropdown-toggle currentStatusBtn" data-id="${ticket.id}" style="background-color: ${SBDashboard.lightenColor(ticket.status_color, 40)};color: black">
                        ${ticket.status_name}
                    </button>
                ...
                <div class="dropdown">
                    <button class="btn btn-sm dropdown-toggle currentPriorityBtn" data-id="${ticket.id}" style="background-color: ${SBDashboard.lightenColor(ticket.priority_color, 40)};color: black">${ticket.priority_name}</button>
        ...
        setupDropdowns: function () {
            const $table = $("table");
            const $dropdownToggles = $table.find(".dropdown-toggle");
            const $dropdownMenus = $table.find(".dropdown-menu");

            function closeAllDropdowns(except = null) {
                $dropdownMenus.each(function () {
                const $menu = $(this);
                if (this !== except) {
                    $menu.removeClass("show").css({ top: "", bottom: "", left: "", right: "" });
                }
                });
                ...
                if (!except || except !== $toggle.next(".dropdown-menu")[0]) {
                    $arrow.removeClass("open");
                }
                });
            }

            $dropdownToggles.each(function (index) {
                $(this).on("click", function (event) {
                event.stopPropagation(); // prevent window click close immediately

                const $toggle = $(this);
                const $dropdownMenu = $dropdownMenus.eq(index);
                ...
                const btnBottomSpace = viewportHeight - btnBottom;

                closeAllDropdowns(isShown ? null : $dropdownMenu[0]);

                if (!isShown) {
                    // Temporarily show dropdown to measure
                    $dropdownMenu.addClass("show").css({ visibility: "hidden", display: "block" });

                    const tableRect = $table[0].getBoundingClientRect();
                    const tableRectTop = tableRect.top;
                    const tableRectRight = tableRect.right;

                    const toggleRect = this.getBoundingClientRect();
                    const toggleRectTop = toggleRect.top;
                    const toggleRectRight = toggleRect.right;

                    const dropdownHeight = $dropdownMenu[0].getBoundingClientRect().height;
                    const dropdownWidth = $dropdownMenu[0].getBoundingClientRect().width;

                    $dropdownMenu.css({ visibility: "", display: "" });

                    // Position dropdown depending on space
                    if (btnBottomSpace < dropdownHeight) {
                    $dropdownMenu.css("bottom", btnBottomSpace + "px");
                    console.log("ok");
                    } else {
                    $dropdownMenu.css("top", btnTop + "px");
                    console.log("not ok");
                    }

                    const btnSpaceCut = dropdownWidth - btnWidth;
                    $dropdownMenu.css("left", btnLeft - btnSpaceCut + "px");

                    // Show dropdown
                    $dropdownMenu.addClass("show");
                    $toggle.find(".arrow").addClass("open");
                } else {
                    $dropdownMenu.removeClass("show");
                    $toggle.find(".arrow").removeClass("open");
                }
                });
            });
...
var SBReports = {
    ...
    var SBConversations = {
        ...
        displayUserscheckList: function()
        {
            ...
            SBF.ajax({
                ...
                let html = '<input type="text" id="userSearch" class="form-control mb-2" style="box-sizing: border-box;" placeholder="Search users...">'; 
                users.forEach((user) => {
    ...
    var SBProfile = {
        ...
        populate: async function (user, profile_area, is_extra = true) {
            ...
            if (user.isExtraEmpty() && is_extra) {
                ...
                profile_area.html(`<ul class="ul-user">${code[0]}</ul>${code[1] ? `<ul class="ul-conversation">${code[1]}</ul>` : ''}${code[2] ? `<ul class="ul-technology">${code[2]}</ul>` : ''}`);
    ...
    */
    ...
    $(document).ready(function () {
        ...
        setTimeout(() => {
        $('.sb-editor').removeClass('sb-active');
        }, 1000);
        admin = $('.sb-admin');
