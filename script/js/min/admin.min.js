    ...
    var SBDashboard = {
        ...
        get: function (onSuccess, ticket_id = false, categories = false, full = true, language = false) {
            ...
            SBF.ajax({
                ...
                console.log('tt00',values1,response[1].resolved_tickets_count,resolvedBeforeLasteWeek,resolvedTicketspercentageIncrease);
...
console.log('hereherte');
                const months2 = Object.keys(response[1].pending_ticket_data);
                // Get values array
                const values2 = Object.values(response[1].pending_ticket_data);
                $('.ticket-pending').html(response[1].pending_tickets_count);

                const lastWeekPendingSum = values2.reduce((acc, curr) => acc + curr, 0);
                const pendingBeforeLasteWeek = response[1].resolved_tickets_count - lastWeekPendingSum == 0 ? 1 : response[1].pending_tickets_count - lastWeekPendingSum;
                const pendingTicketspercentageIncrease = (lastWeekPendingSum/pendingBeforeLasteWeek)*100;

                console.log('pending',values2,response[1].pending_tickets_count,pendingBeforeLasteWeek,pendingTicketspercentageIncrease);

                $('.total-resolved-tickets-increase').html(this.formatNumber(pendingTicketspercentageIncrease));

                const ticket_pendingCtx = document.getElementById('ticket_pending_chart').getContext('2d');
                const gradient5 = ticket_pendingCtx.createLinearGradient(0, 0, 0, 200);
                gradient5.addColorStop(0, 'rgba(6, 131, 255, 0.2)');
                gradient5.addColorStop(1, 'rgba(6, 131, 255, 0)');
    ...
    */
    ...
    var SBTicket = {
        ...
        fetchUsers : async function () {
            try {
                console.log('users'); // now `users` is ready for use
