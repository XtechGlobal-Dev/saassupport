    ...
    var SBF = {
        ...
        escape: function (input) {
            if (typeof input === 'string') {
                return input.replace(/</ig, '&lt;').replace(/javascript:|on\w+/ig, '');
            } else if (Array.isArray(input)) {
                return input.map(str => typeof str === 'string' ? str.replace(/</ig, '&lt;').replace(/javascript:|on\w+/ig, '') : '');
            } else if (typeof input === 'object' && input !== null) {
                const escaped = {};
                for (const key in input) {
                    escaped[key] = typeof input[key] === 'string' ? input[key].replace(/</ig, '&lt;').replace(/javascript:|on\w+/ig, '') : input[key];
                }
                return escaped;
            }
            return '';
    ...
    */
    ...
    $.fn.setProfile = function (name = false, profile_image = false) {
        ...
        profileBlocks.each(function () {
            ...
            profile.find('img').attr('src', profile_image).attr('data-name', name || '');
            profile.find('.sb-name').text(name || 'Unknown User');
            const pro = profileBlocks.find(".sb-profile");
            const img = profileBlocks.find(".avatar_img");
            const initials = name.split(" ").map(word => word[0]).join("").substring(0, 2).toUpperCase();
            const avatar_name = profileBlocks.find(".avatar_name");
            const avatar_initials = profileBlocks.find('.avatar_initials');
        ...
        populateConversations: function (onSuccess = false) {
            if (!this.is_busy_populate && activeUser()) {
                ...
                activeUser().getConversations((response) => {
                    ...
                    console.log('Conversations populated', response);
                });
            }
        },
