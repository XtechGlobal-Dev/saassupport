...
function sb_send_message($sender_id, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false, $recipient_id = false) {
    ...
    if ($sender_id) {
        ...
        $is_human_takeover = $is_chatbot_active && !$is_human_takeover_active && $conversation_source != 'tm' && $conversation_source != 'em' && sb_get_multi_setting('dialogflow-human-takeover', 'dialogflow-human-takeover-active');
        $last_agent = false;
...
function sb_rating_message($conversation_id) {
    ...
    $message_id = sb_send_message(sb_get_bot_ID(), $conversation_id, $message, [], 3, ['rating_message' => true])['id'];
    return sb_messaging_platforms_send_message($message, $conversation_id, $message_id);
}
    ...
    if (in_array($name, ['registration', 'registration-tickets', 'login', 'login-tickets', 'timetable', 'articles', 'woocommerce-cart'])) {
        ...
        switch ($name) {
            ...
            case 'registration':
                ...
                $password = (!$registration_tickets && sb_get_setting('registration-required') == 'registration-login') || ($registration_tickets && !sb_get_setting('tickets-registration-disable-password')) ? '<div id="password" data-type="text" class="sb-input sb-input-password" style="position: relative;"><span>' . sb_('Password') . '</span>
                <svg id="chatRegPass" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#000000" fill="none" style="position:absolute; right: 10px;cursor:pointer;" role="button" tabindex="0" aria-label="Toggle password visibility">
                    <path d="M21.544 11.045C21.848 11.4713 22 11.6845 22 12C22 12.3155 21.848 12.5287 21.544 12.955C20.1779 14.8706 16.6892 19 12 19C7.31078 19 3.8221 14.8706 2.45604 12.955C2.15201 12.5287 2 12.3155 2 12C2 11.6845 2.15201 11.4713 2.45604 11.045C3.8221 9.12944 7.31078 5 12 5C16.6892 5 20.1779 9.12944 21.544 11.045Z" stroke="#141B34" stroke-width="1.5"></path>
                ...
                <input value="' . ($user && $user['password'] ? '********' : '') . '" autocomplete="false" type="password" id="chatRegInput" required> </div><div id="password-check" data-type="text" class="sb-input sb-input-password" style="position: relative;"><span>' . sb_('Repeat password') . '</span><svg id="chatRegRepeatPass" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#000000" fill="none" style="position:absolute; right: 10px;cursor:pointer;" role="button" tabindex="0" aria-label="Toggle password visibility">
                    <path d="M21.544 11.045C21.848 11.4713 22 11.6845 22 12C22 12.3155 21.848 12.5287 21.544 12.955C20.1779 14.8706 16.6892 19 12 19C7.31078 19 3.8221 14.8706 2.45604 12.955C2.15201 12.5287 2 12.3155 2 12C2 11.6845 2.15201 11.4713 2.45604 11.045C3.8221 9.12944 7.31078 5 12 5C16.6892 5 20.1779 9.12944 21.544 11.045Z" stroke="#141B34" stroke-width="1.5"></path>
                    <path d="M15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15C13.6569 15 15 13.6569 15 12Z" stroke="#141B34" stroke-width="1.5"></path>
            ...
            case 'login':
                ...
                $code = '<div class="sb-form">
                            <div id="email" class="sb-input"><span>' . sb_('Email') . '</span><input autocomplete="false"
                                    type="email"> </div>
                            <div id="password" class="sb-input" style="position:relative;">
                                <span>' . sb_('Password') . '</span>
                                <svg id="ticketRegPass" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                                    height="24" color="#000000" fill="none"
                                    style="position:absolute; right: 10px;" role="button" tabindex="0"
                                    aria-label="Toggle password visibility">
                                    <path
                                        d="M21.544 11.045C21.848 11.4713 22 11.6845 22 12C22 12.3155 21.848 12.5287 21.544 12.955C20.1779 14.8706 16.6892 19 12 19C7.31078 19 3.8221 14.8706 2.45604 12.955C2.15201 12.5287 2 12.3155 2 12C2 11.6845 2.15201 11.4713 2.45604 11.045C3.8221 9.12944 7.31078 5 12 5C16.6892 5 20.1779 9.12944 21.544 11.045Z"
                                        stroke="#141B34" stroke-width="1.5"></path>
                                    <path
                                        d="M15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15C13.6569 15 15 13.6569 15 12Z"
                                        stroke="#141B34" stroke-width="1.5"></path>
                                </svg>
                                <input autocomplete="off" type="password" id="ticketRegInput">
                                
                            </div>
                        </div>
                        <div class="sb-buttons">
                            <div class="sb-btn sb-submit-login">' . sb_('Sign in') . '</div>' .
                    (sb_get_setting('registration-required') == 'login' ? '' : '<div
                                class="sb-btn-text sb-registration-area">' . sb_('Create new account') . '</div>') . '
                        </div>';
                break;
            case 'timetable':
...
function sb_messaging_platforms_functions($conversation_id, $message, $attachments, $user, $source) {
    ...
    if (($dialogflow_active || $open_ai_active) && !sb_get_setting(($source_name == 'ig' ? 'fb' : $source_name) . '-disable-chatbot')) {
        ...
        if (!$skip_chatbot && (!sb_get_setting('dialogflow-timetable') || !sb_office_hours())) {
            ...
            if ($dialogflow_active) {
                $response = sb_dialogflow_message($conversation_id, $message, [sb_get_user_language($user_id)], $attachments, '', false, false, false, $voice_message);
            ...
            for ($i = 0; $i < count($messages); $i++) {
                ...
                if ($message_text || $message_attachments) {
                    ...
                    sb_save_external_setting('stop-worker-' . $conversation_id, 'true');
                }
                if ($payload) {
    ...
    if ($bot_messages || $human_takeover) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($message_) {
                $bot_message = ['id' => sb_send_message(sb_get_bot_ID(), $conversation_id, $message_)['id'], 'message' => $message_];
            }
            if ($bot_message) {
    ...
    // Rating
    if ($open_ai_active && str_contains(sb_isset($last_message, 'payload'), 'rating_message')) {
        ...
        if ($rating == 'positive' || $rating == 'negative') {
            sb_set_rating($conversation_id, $user_id, $rating == 'positive' ? 1 : -1, $message);
        }
    }
...
function sb_send_sms($message, $to, $template = true, $conversation_id = true, $attachments = false) {
    ...
    $to_agents = $to == 'agents' || $to == 'all-agents' || str_contains($to, 'department-');

    // Retrive phone number
    if ($to_agents) {
        $phones = sb_db_get('SELECT A.id, value FROM sb_users A, sb_users_data B WHERE A.id = B.user_id AND (user_type = "agent" OR user_type = "admin") AND slug = "phone"' . ($to == 'agents' ? ' AND (department IS NULL OR department = "")' : (str_contains($to, 'department-') ? ' AND department = ' . substr($to, 11) : '')), false);
