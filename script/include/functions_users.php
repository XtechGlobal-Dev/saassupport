...
function sb_get_tickets($ticket_status, $sorting = ['t.creation_time', 'DESC'], $search = '', $pagination = 0, $extra = false, $post_ids = false, $department = false, $tag = false, $tags = false, $source = false) {
    ...
    if ($post_ids) {
        ...
        if ($count_post_ids) {
            $query .= ($query ? ' AND ' : '') . ' sb_tickets.id IN (' . sb_db_escape(implode(',', $post_ids)) . ')';
        }
    }
    if ($department || $tag || $source) {
        $query .= ($query ? ' AND ' : '') . ' sb_tickets.id IN (SELECT A.id FROM sb_users A, sb_conversations B WHERE A.id = B.user_id' . ($department ? ' AND B.department = "' . sb_db_escape($department, true) . '"' : '') . ($tag ? ' AND B.tags LIKE "%' . sb_db_escape($tag) . '%"' : '') . ($source ? ' AND ' . ($source == 'chat' ? '(B.source = "" OR B.source IS NULL)' : 'B.source = "' . sb_db_escape($source) . '"') : '') . ')';
    }*/
    /*if ($search) {
        $searched_users = sb_search_users($search);
        $count_search = count($searched_users);
...
function get_total_users_count($start_date = null,$end_date = null)
{
    ...
    $query = 'SELECT count(id) as total_users_count FROM sb_users  WHERE user_type = "user"';
    $totalUsers = sb_db_get($query,true);
...
{
    if ($conversation_id) {
        sb_db_query('UPDATE sb_conversations SET converted_to_ticket = 1 WHERE id = ' . $conversation_id);
        
        $conversion = sb_db_get('SELECT sbu.first_name, sbu.last_name FROM sb_users as sbu INNER JOIN sb_conversations sbc on sbc.user_id = sbu.id  WHERE sbc.id = ' . $conversation_id);
        $subject = $conversion['first_name'] . ' ' . $conversion['last_name'];
        $description = 'Conversation #' . $conversation_id . ' has been converted to ticket.';

       $values = 'VALUES  (\''.$subject."', '".$description."', '".$conversation_id."', '".sb_gmt_now()."')";
        //echo 'INSERT into sb_tickets(subject,description,conversation_id,creation_time) '.$values;
       sb_db_query('INSERT into sb_tickets(subject,description,conversation_id,creation_time) '.$values);
...
function update_ticket_detail($inputs,$ticket_id =0){
    $ticket_id = sb_db_escape($ticket_id, true);
    $withoutContact = isset($inputs['withoutContact'][0]) ? sb_db_escape($inputs['withoutContact'][0],true) : false;
    $data = [
            //'contact_id' => $withoutContact ? 0 : $inputs['contact_id'][0],
            'assigned_to' => sb_db_escape($inputs['assignee'],true),
            //'contact_name' => sb_db_escape($inputs['cust_name'][0]),
            //'contact_email' => sb_db_escape($inputs['cust_email'][0]),
            'priority_id' => sb_db_escape($inputs['priority'],true),
            //'service_id' => sb_db_escape($inputs['service_id'][0],true),
    ...
    $customFields = array();
    if(isset($inputs['customField']) && is_array($inputs['customField']))
    {
        foreach($inputs['customField'] as $key => $value) {
            $customFields[$key] = sb_db_escape($value);
        }
    }
...
function update_ticket_subject($subject,$ticket_id)
{
...
{
    $ticket_id = sb_db_escape($ticket_id, true);
    sb_save_ticket_attachments($ticket_id, $attachments);
}

function update_ticket_status($ticket_id = null, $status = null)
{
    $ticket_id = sb_db_escape($ticket_id, true);
    $status = sb_db_escape($status, true);

    // Update the comment in the database
    $query = "UPDATE sb_tickets SET status_id = $status WHERE id = $ticket_id";
    $result = sb_db_query($query);
