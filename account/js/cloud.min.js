...
(function ($) {
    ...
    let messages = {
        ...
        terms: sb_('You must accept the Terms of Services and Privacy Policy.'),
    };
    ...
    $(document).ready(function () {
        ...
        if (box_account.length) {
            ...
            ajax('account-user-details', {}, (response) => {
                account = response;
                setTimeout(function () {
                    box_account.find('#embed-code2').val(`<!-- ${BRAND_NAME} -->\n<script id="chat-init" src="${CLOUD_URL}/account/js/init.js?id=${response.chat_id}&mode=tickets"></script>`);
            ...
            if (URL.includes('welcome') && SETTINGS.text_welcome_title) {
                ...
                box_account.find('.sb-btn-dashboard').addClass('animation-button').attr('href', CLOUD_URL + '/?area=dashboard').attr('target', '_blank');
            }
        ...
        if (box_registration.length) {
            ...
            function setFieldError(wrapper, message) {
                wrapper.find('.error-msg').html(message);
                wrapper.find('input').addClass('sb-error');
            }

            function clearFieldError(wrapper) {
                wrapper.find('.error-msg').html('');
                wrapper.find('input').removeClass('sb-error');
            }
            ...
            $(box_registration).on('click', '.btn-register', function (e) {
                ...
                box_registration.find('[id].sb-input').each(function () {
                    ...
                    const fieldId = wrapper.attr('id');
                    const value = $.trim(input.val());

                    clearFieldError(wrapper); // clear previous errors for this field
                    details[fieldId] = value;

                    if (!value) {
                        setFieldError(wrapper, sb_('This field is required.'));
                        errors = true;
                    } else if (fieldId === 'email' && !isValidEmailFormat(value)) {
                        setFieldError(wrapper, sb_('Please enter a valid email address (e.g. name@domain.com).'));
                        errors = true;
                    }
                });
                ...
                const terms = $('#terms');
                if (!terms.is(':checked')) {
                    terms.parent().addClass('sb-error');
                    errors = true;
                } else {
                    terms.parent().removeClass('sb-error');
                }

                // Additional validations if no missing fields so far
                if (!errors) {
                    if (details['password'].length < 8) {
                        setFieldError($('#password'), sb_('Password must be at least 8 characters long.'));
                        errors = true;
                    }
                    if (details['password'] !== details['password_2']) {
                        setFieldError($('#password_2'), sb_('Passwords do not match.'));
                        errors = true;
                    }
                }
                ...
                ajax('registration', { details }, (response) => {
                    if (response === 'duplicate-email') {
                        setFieldError($('#email'), sb_('This email is already in use. Please use another email.'));
                    } else {
                        setLogin(response[0], response[1]);
                        ajax('account-welcome');
                        setTimeout(() => {
                            document.location = SBF.getURL('redirect')
                                ? SBF.getURL('redirect')
                                : CLOUD_URL + '/account?welcome';
                        }, 300);
                    }
                    $(this).stopLoading();
                });
            ...
            $(box_login).on('click', '.btn-login', function (e) {
                ...
                let errors_area = box_login.find('.error');
                let emailErr = box_login.find('.emailerror');
                let passworderror = box_login.find('.passworderror');
                ...
                if (!email) {
                    emailErr.html(sb_('Email field cannot be empty.'));
                    $('#businessemail').addClass('sb-error');
                    hasError = true;

                }
                const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                if (!emailRegex.test(email)) {
                    emailErr.html(sb_('Please enter a valid email address.'));
                    $('#businessemail').addClass('sb-error');
                    hasError = true;
                }

                if (!password) {
                    passworderror.html(sb_('Password field cannot be empty.'));
                    $('#passwordfield').addClass('sb-error');
                    hasError = true;
                }
                if (password.length < 8) {
                    passworderror.html(sb_('Password must be at least 8 characters long.'));
                    $('#passwordfield').addClass('sb-error');
                    hasError = true;
                }
                if (hasError || loading(this)) {
                    return false;
                }
            ...
            $(box_reset_password).on('click', '.btn-reset-password', function () {
                let email = $.trim($('#reset-password-email').val());
                if (!email) {
                    $('#reset-password-email').addClass('sb-error');
                    return false;
                } else {
                    if (email && email.includes('@') && email.includes('.')) {
                        ...
                        $('.forgot-password-section').html(`<div class="sb-top-bar"><div class="sb-title">${sb_('Check your email')}</div><div class="sb-text">${sb_('A password reset link has been sent to your registered email address. Please check your inbox and spam folder.')}</div></div>`);
                    }
                    else {
                        $('#reset-password-email').addClass('sb-error');
                        return false;
    ...
    function banner(title, message, code = '', image = false, error = false, success = false) {
        ...
        body.find('.sb-tab > .sb-content > .sb-active').prepend(`<div 11 id="banner-${id}" ${id == 'welcome-aboard' ? 'style="height: 171px;"' : ''} ' banner-img' : ''} class="banner${image ? ' banner-img' : ''}${error ? ' banner-error' : ''}${success ? ' banner-success' : ''}">${image ? `<img src="${image}" />` : ''}<h2>${sb_(title)}</h2><p>${sb_(message)}</p><div>${code}</div><i class="sb-btn-icon sb-icon sb-icon-close"></i></div>`);

        if(!id)
        {
            body.find(`#banner-${id} .sb-icon-close`).remove();
            body.find(`#banner-${id}`).attr('style','padding:10px;margin-left: auto');
            setTimeout(() => {
              body.find(`#banner-${id}`).remove();
            },1800);
        }
    ...
    // Add Eye Icon on Password and toggle visibility
    ...
    $('#toggleResetRepeatPassword').click(function () {
        $(this).toggleClass("fa-eye fa-eye-slash");
        let input = $("#reset-password-2");
        let currentType = input.prop('type');
        if (currentType === 'password') {
            input.prop('type', 'text');
        } else {
    ...
    $('#togglePasswordsignup').click(function () {
        $(this).toggleClass("fa-eye fa-eye-slash");
        let input = $("#password-field-signup");
        let currentType = input.prop('type');
        if (currentType === 'password') {
            input.prop('type', 'text');
        } else {
    ...
    $('#togglePasswordconfirm').click(function () {
        $(this).toggleClass("fa-eye fa-eye-slash");
        let input = $("#password-field-confirm");
        let currentType = input.prop('type');
        if (currentType === 'password') {
            input.prop('type', 'text');
        } else {
    ...
    $(document).on('click', '.copy-button', function () {
        const $button = $(this);
        const $textarea = $button.prev('.embed-code');
        ...
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(textToCopy).then(() => {
                ...
                fallbackCopy($textarea[0], $tooltip);
            });
        } else {
            fallbackCopy($textarea[0], $tooltip);
        }
    ...
    function fallbackCopy(textarea, $tooltip) {
        textarea.select();
